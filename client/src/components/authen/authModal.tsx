...
export default function AuthModal() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.response?.data?.message || "เข้าสู่ระบบไม่สำเร็จ โปรดตรวจสอบอีเมลและรหัสผ่าน",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.response?.data?.message || "สมัครสมาชิกไม่สำเร็จ โปรดลองใหม่อีกครั้ง",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 lg:p-0 font-sans selection:bg-[#10B981] selection:text-white">
      ...
      {/* Auth Container */}
      <div className="bg-white w-full max-w-250 min-h-162.5 rounded-[10px] shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 animate-in fade-in zoom-in-95 duration-700">
        
        {/* Left Side: Branding & Visuals (ใช้งานร่วมกันทั้ง Login และ Register) */}
        ...
        <div className="p-8 lg:p-16 flex flex-col justify-center bg-white relative">
          ...
          {/* แสดง Message แจ้งเตือน ถ้ามี */}
          {message && (
            <div className={`mb-4 p-3 rounded text-[12px] font-bold ${message.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {message.text}
            </div>
          )}

          {/* สลับฟังก์ชัน onSubmit ตามหน้าปัจจุบัน */}
          <form onSubmit={isLoginView ? handleAuth : handleRegister} className="space-y-4">
            
            {/* โชว์ฟิลด์ Username เฉพาะตอนหน้า Register (!isLoginView) */}
            {!isLoginView && (
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center px-1">
                ...
                {/* โชว์ลืมรหัสผ่านเฉพาะหน้า Login */}
                {isLoginView && (
                  <button
                    type="button"
            ...
            >
              {isLoading ? (
                ...
                <>
                  {isLoginView ? "Authenticate" : "Initialize Node"}
                  <ArrowRight
                    size={14}
                    className="group-hover:translate-x-1 transition-transform"
                  />
          ...
          <div className="mt-8">
            <div className="relative flex items-center justify-center mb-6">
              ...
              <span className="bg-white px-4 text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em] absolute">
                Global Access
              </span>
            </div>
          ...
          {/* สลับปุ่มสลับหน้าระหว่าง Login/Register */}
