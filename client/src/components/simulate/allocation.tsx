...
export const Asset = ({}) => {
    const [allocations, setAllocations] = useState([
      ...
      const updateWeight = (id, val) => {
        setAllocations(allocations.map(a => a.id === id ? { ...a, weight: val } : a));
      };
    ...
    <div className="lg:col-span-4 flex flex-col gap-6">
               <div className="bg-white rounded-[10px] border border-slate-200 p-8 shadow-sm flex flex-col h-full text-slate-900">
                  <div className="flex justify-between items-start mb-10">
                    ...
                    <button className="cursor-pointer bg-black text-white px-6 py-2 rounded-[6px] text-[9px] font-bold uppercase flex items-center gap-1 shadow-lg mt-1.5">
                    ...
                    {allocations.map((item) => (
                      <div key={item.id} className="grid grid-cols-12 gap-3 items-center bg-slate-50 p-3 rounded-[10px] group transition-all hover:bg-slate-100/50">
                        <div className="col-span-7 relative">
                           <select className="w-full bg-white border border-slate-200 rounded-[6px] py-2 px-3 text-xs font-bold appearance-none cursor-pointer outline-none focus:border-black">
                        ...
                        <div className="col-span-4">
                           <input 
                              ...
                              className="w-full bg-white border border-slate-200 rounded-[6px] py-2 px-3 text-center text-xs font-bold outline-none focus:border-black" 
                           />
                        </div>
                  ...
                  <div className="mt-10 pt-10 border-t border-slate-100">
                    ...
                    <button 
                      ...
                      className={`w-full py-4 rounded-[8px] font-bold uppercase text-xs flex items-center justify-center gap-3  ${
